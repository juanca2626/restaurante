...
@php
    $widgets['before_content'][] = [
        'type'        => 'jumbotron',
        'heading'     => 'Pedidos por cliente',
        'content'     => 'Aquí puede ver tus reportes de los pedidos que han hecho tus clientes',
    ];
@endphp
...
@section('content')
  ...
  <div>
    <div class="mb-5 d-flex gap-2 justify-content-between">
        <a href="{{ route('dashboard') }}" class="btn btn-primary">Ir a Pedidos por Día</a>
      ...
      <table class="table table-hover table-stripped">
        ...
        <tbody>            
            @foreach ($orders->groupBy('customer_id')->values() as $groupedOrders)            
                <tr>
                    <td>{{ $groupedOrders->first()->customer->name }} - {{ $groupedOrders->first()->customer->cellphone }}</td>
                    @foreach ($days as $index => $day)
                        @php
                            $filteredOrders = $groupedOrders->filter(fn($x) => $x->delivery_date->dayOfWeek == $index+1)->values();
                        @endphp
