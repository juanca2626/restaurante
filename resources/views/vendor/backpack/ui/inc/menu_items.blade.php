{{-- This file is used for menu items by any Backpack v6 theme --}}
<li class="nav-item"><a class="nav-link" href="{{ backpack_url('dashboard') }}"><i class="la la-home nav-icon"></i> Dashboard</a></li>

<x-backpack::menu-item title="Menu" icon="la la-question" :link="backpack_url('menu')" />
<x-backpack::menu-item title="Platos" icon="la la-question" :link="backpack_url('dish')" />
{{-- <x-backpack::menu-item title="Orders" icon="la la-question" :link="backpack_url('order')" /> --}}